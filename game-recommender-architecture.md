...
| LLM | **Claude API (Anthropic)** — used two different ways | (1) Batch enrichment: difficulty / story-vs-gameplay / content warnings, cached per game. (2) Online: per-candidate "why/why not" explanations on the ~15–20 shortlisted games only. |
...
3. **LLM enrichment job** — for each game missing enrichment, one Claude call to infer difficulty, story-vs-gameplay ratio, and content-warning tags from IGDB summary + genre/theme + (optionally) scraped review snippets. Store with a `enriched_at` timestamp and a schema version, so you can re-run cheaply when you improve the prompt later.
...
2. FastAPI computes **weighted soft-match score** in-process (length distance, review score, difficulty distance, popularity/nichety distance, story/gameplay distance, similarity score) → ranks candidates.
...
games
  ...
  difficulty_score, story_gameplay_ratio   -- LLM-enriched
